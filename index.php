...
// 1. AMBIL REKOR TERCEPAT SECARA DINAMIS DARI DATABASE
$query_rekor = mysqli_query($koneksi, "
...
// Set data default jika database masih kosong
$has_record = false;
$record_time = "15.351";
$sector_1 = "4.893";
$sector_2 = "5.371";
...
if ($query_rekor && mysqli_num_rows($query_rekor) > 0) {
    $has_record = true;
    $data_rekor = mysqli_fetch_assoc($query_rekor);
    
    // Dipaksa menggunakan titik ('.') sebagai pemisah desimal agar fungsi explode tidak error
    $record_time = number_format($data_rekor['total_lap'], 3, '.', '');
    $sector_1 = number_format($data_rekor['sektor_1'], 3, '.', '');
    $sector_2 = number_format($data_rekor['sektor_2'], 3, '.', '');
    $sector_3 = number_format($data_rekor['sektor_3'], 3, '.', '');
    $driver_name = $data_rekor['nama_lengkap'];
}

// 2. AMBIL DATA PILIHAN PAKET BERMAIN UNTUK DI-SHOWCASE
$query_paket = mysqli_query($koneksi, "SELECT * FROM paket_bermain ORDER BY harga ASC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<body>
    ...
    <div class="hero">
        ...
        <div class="hero-cta-container">
            <a href="login.php" class="hero-login-btn">🏁 BOOK YOUR RACE - START LOGIN</a>
        </div>
    ...
    <div id="specs" class="panel">
        ...
        <div class="power-curve-placeholder">
    ...
    <div class="price-container-panel">
        <h2 class="panel-header" style="text-align: center; font-family: 'Orbitron', sans-serif; color: #fff;">Pilihan Paket Bermain</h2>
        <p class="panel-desc" style="text-align: center; color: #9ca3af; margin-bottom: 3.5rem;">Tarif durasi sewa gokart transponder yang tersedia</p>
        
        <div class="price-grid">
            <?php while($paket = mysqli_fetch_assoc($query_paket)): ?>
                <div class="price-card">
                    <div class="package-badge">GOKART HUB</div>
                    <h3><?= htmlspecialchars($paket['nama_paket']); ?></h3>
                    <p class="desc"><?= htmlspecialchars($paket['deskripsi']); ?></p>
                    <div class="cost">
                        <span class="currency">Rp</span>
                        <span class="amount"><?= number_format($paket['harga'], 0, ',', '.'); ?></span>
                    </div>
                    <div class="details-spec">
                        <div class="spec-line">⏱️ <span>Durasi:</span> <strong><?= $paket['durasi_menit']; ?> Menit</strong></div>
                        <div class="spec-line">👤 <span>Kapasitas:</span> <strong><?= $paket['maksimal_orang']; ?> Driver</strong></div>
                    </div>
                </div>
            <?php endwhile; ?>
        </div>
    ...
    <div class="panel">
        ...
        <div class="race-track-map">
            <svg id="track-svg" viewBox="0 0 1000 500" preserveAspectRatio="none">
                ...
                <g id="car-group">
                    <image href="kart.png" x="-30" y="-35" width="60" height="60" />
    ...
    <div class="panel">
        ...
        <?php if($has_record): 
            $split_time = explode('.', $record_time);
        ?>
            <div class="lap-record"><?= $split_time[0]; ?><span class="red">.<?= $split_time[1]; ?></span></div>
            <div class="lap-record-sub">seconds</div>
            <div class="record-driver-info">Set by <strong><?= htmlspecialchars($driver_name); ?></strong></div>
        <?php else: ?>
    ...
    <div class="panel">
        ...
        <div class="flags-grid">
            ...
            <div class="flag-box black"><div class="flag-header"><div class="flag-color white-bg"></div><h4 class="flag-name">Bendera Hitam</h4></div><p class="flag-desc">Driver diskualifikasi atau harus kembali ke pit. Penalty dikeluarkan.</p></div>
        </div>
    </div>
    ...
    <div class="panel">
        ...
        <div class="facilities-grid">
            <div class="facility-box">
                <div class="facility-icon-container">🛋️</div>
                <div><h4 class="facility-title">Kursi Penonton</h4><p class="facility-desc">Tribun tertutup dengan visibilitas trek penuh untuk hingga 200 tamu.</p></div>
            </div>
            <div class="facility-box">
                <div class="facility-icon-container">🔧</div>
                <div><h4 class="facility-title">Area Pit Stop</h4><p class="facility-desc">Teluk yang dilengkapi untuk layanan kart, ganti ban, dan perbaikan cepat.</p></div>
            </div>
            <div class="facility-box">
                <div class="facility-icon-container">🛋️</div>
                <div><h4 class="facility-title">Lounge Driver</h4><p class="facility-desc">Lounge ber-AC dengan monitor yang menampilkan waktu putaran langsung.</p></div>
            </div>
            <div class="facility-box">
                <div class="facility-icon-container">☕</div>
                <div><h4 class="facility-title">Kafe & Segaran</h4><p class="facility-desc">Kafe di tempat yang menyajikan minuman panas dan dingin, makanan ringan, dan hidangan.</p></div>
            </div>
            <div class="facility-box">
                <div class="facility-icon-container">🛡️</div>
                <div><h4 class="facility-title">Ruang Briefing</h4><p class="facility-desc">Ruang khusus untuk orientasi keselamatan pra-balapan wajib.</p></div>
            </div>
            <div class="facility-box">
                <div class="facility-icon-container">👷</div>
                <div><h4 class="facility-title">Sewa Gear & Helm</h4><p class="facility-desc">Berbagai macam helm, setelan, dan sarung tangan tersedia dalam semua ukuran.</p></div>
            </div>
        </div>
    </div>

    <footer>
        © 2026 Go-Kart Racing Hub. All rights reserved.
    </footer>
